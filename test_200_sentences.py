...
def run_tests():
    ...
    for i, sentence in enumerate(test_sentences, 1):
        try:
            date_result, time_result = parser_date_time(sentence, time_format_type=2)  # 預設 hh:mm
            results.append((i, sentence, f"{date_result},{time_result}"))
            print(f"{i:2d}. {sentence} -> {date_result},{time_result}")
    ...
    # 統計結果
    success_count = sum(1 for _, _, result in results if not result.startswith("錯誤"))
